...
server.route({
  ...
  config: {
    validate: {
      payload: {
        redirectLocation: Joi.string().uri({
          scheme: ['http', 'https']
        }).required(),
  ...
  handler: function(request, reply) {
    ...
    if (!isRedirectLocationAllowed(redirectLocation)) {
      reply(Boom.badRequest('unallowed redirect location'));
      return;
    }
    ...
    db.get('org.couchdb.user:' + email, function(err, data) {
      ...
      doc.couch_email_auth_redirect_location = redirectLocation,
  ...
  handler: function(request, reply) {
    db.get('org.couchdb.user:' + request.query.email, function(err, doc) {
      ...
      nano.auth(request.query.email, password, function(err, body, headers) {
        ...
        // we need this
        redirectLocation = doc.couch_email_auth_redirect_location;
        // but we don't want to keep this
        delete doc.couch_email_auth_redirect_location;
...
function isRedirectLocationAllowed(redirectLocation) {
  if (config.allowedRedirectHosts.length === 0) {
    return true;
  }

  var host = url.parse(redirectLocation).host;
