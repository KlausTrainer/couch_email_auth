  ...
  config: {
    validate: {
      ...
      failAction: function(request, reply, source, err) {
        ...
        } else if (/token.*required/.test(err.message)) {
          error = Boom.badRequest();
          error.output.payload = '{"error":"missing \'token\' query parameter"}';
        } else if (/child "token" fails.*/.test(err.message)) {
          error = Boom.badRequest();
          error.output.payload = '{"error":"invalid token"}';
        } else if (/valid.*email/.test(err.message)) {
          error = Boom.badRequest();
          error.output.payload = '{"error":"invalid email"}';
        } else if (/token.*email/.test(err.message)) {
          error = Boom.badRequest();
          error.output.payload = '{"error":"invalid email"}';
        } else {
          error = Boom.badRequest();
