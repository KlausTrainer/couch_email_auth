  ...
  handler: function(request, reply) {
    ...
    db.get('org.couchdb.user:' + email, function(err, data) {
      ...
      if (err && err.error === 'not_found') {
        ...
        reply(Boom.badImplementation('{"error":"database connection error"}'));
        return;
      } else {
        doc = data;
      }
      ...
      db.insert(doc, function(err, d) {
        ...
        if (err) {
          reply(
            Boom.badImplementation('{"error":"database connection error"}'));
          return;
        }
          ...
          if (error) {
            reply(
              Boom.badImplementation(
                '{"error":"could not send login credentials"}'));
            return;
          }
          reply({ok: "true"});
        });
